...
func main() {
	...
	router.HandleFunc("/rfp/decline", hotelInit.DeclineRFP)           //Decline rfp by hotel
	router.HandleFunc("/hotelDetail", hotelInit.HotelDetails)         // hotel info
	router.HandleFunc("/hotel/compEngaged", hotelInit.EngagedCompany) //get hotel details for review
	...
	router.HandleFunc("/rfp/draft", companyInit.RfpDrafted)
	...
	router.HandleFunc("/rfp/shortlistQuotes", companyInit.ShortListQuotes)     //shortlist quotes
	router.HandleFunc("/rfp/unShortlistQuotes", companyInit.UnShortListQuotes) //unshortlist quotes
	router.HandleFunc("/rfp/hotelRes", companyInit.RfpHotelResponse)           //hotel response for particular rfp
	router.HandleFunc("/rfp/declineQuote", companyInit.DeclineQuote)           //declining single quote recieved for rfp
	router.HandleFunc("/rfp/acceptQuote", companyInit.AcceptQuote)             //accept quote from hotel/ connect to corporate
	router.HandleFunc("/rfp/trash", companyInit.TrashRfp)                      //moving rfp to trash before recieving quotes
